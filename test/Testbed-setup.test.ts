...
describe("Testbed scenario", function () {
    ...
    describe("3. ChoreographyNMT - initialization", function () {
        ...
        it("should update with 2 participants, Supplier and CanteenManagement, and emit the StateChanged event", async function () {
            ...
            console.log("AAAB", descriptorAfterSuppliers);

            // Check that each element in the arrays is equal
            for (let i = 0; i < descriptorAfterSuppliers["participants"].length; i++) {
                expect(descriptorAfterSuppliers["participants"][i]).to.equal(participants[i]);
            }
            ...
            expect(eventSuppliers?.args?.choreographyDescriptor.participants[0]).to.deep.equal(participants);
        });
